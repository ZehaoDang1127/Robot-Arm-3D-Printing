...
from robotic_printing_platform.extrusion import FlowSchedule, MaterialProfile
from robotic_printing_platform.robots.generic import RobotTrajectory, _tool_transform
...
def _tcp_anchor_candidates(traj: RobotTrajectory) -> tuple[dict[str, object], ...]:
    """Describe simulator link frames that can locate the configured nozzle TCP.

    Isaac assets sometimes merge fixed URDF links such as ``tool0``.  Export
    every frame in the fixed suffix after the final actuated joint, together
    with that frame's transform to the TCP, so replay can select the first link
    actually present in the loaded USD.
    """
    config = traj.config
    tool_transform = _tool_transform(
        config.tool_length_m,
        config.tool_tcp_xyz_m,
        config.tool_tcp_rpy_rad,
    )

    def candidate(link_name: str, transform: np.ndarray) -> dict[str, object]:
        return {
            "link_name": link_name,
            "translation_m": tuple(float(value) for value in transform[:3, 3]),
            "rotation_xyzw": _rotation_matrix_to_xyzw(transform[:3, :3]),
        }

    # A manually constructed trajectory may omit a URDF.  Keeping the end-link
    # candidate still makes the export useful and produces a clear runtime
    # error if that frame is not present in the simulator asset.
    fallback = (candidate(config.end_link, tool_transform),)
    if not config.urdf_path or not config.end_link:
        return fallback
    try:
        chain = load_urdf_chain(
            config.urdf_path,
            base_link=config.base_link or None,
            end_link=config.end_link,
        )
        zero_q = np.zeros(len(chain.active_joints), dtype=float)
        end_transform, frames = chain.fk(zero_q)
    except (OSError, ValueError):
        return fallback

    active_indices = [index for index, joint in enumerate(chain.joints) if joint.active]
    if not active_indices:
        return fallback
    tcp_transform = end_transform @ tool_transform
    candidates: list[dict[str, object]] = []
    seen_links: set[str] = set()
    # frames[index + 1] is the child-link transform after chain.joints[index].
    for index in range(len(chain.joints) - 1, active_indices[-1] - 1, -1):
        link_name = chain.joints[index].child
        if link_name in seen_links:
            continue
        anchor_to_tcp = np.linalg.inv(frames[index + 1]) @ tcp_transform
        candidates.append(candidate(link_name, anchor_to_tcp))
...
TCP_ANCHOR_CANDIDATES = {tcp_anchor_candidates}
...
def export_isaac_bundle(
    ...
    bed_center_xyz_m: tuple[float, float, float] | None = None,
) -> dict[str, Path]:
    # Fail during export, rather than after Isaac starts, if incoming segment
    # volume cannot form a valid piecewise material-flow schedule.
    FlowSchedule.from_trajectory_points(traj)
    out = Path(output_dir)
    ...
    density_kg_m3 = (
        ...
        else material.density_g_cm3 * 1000.0
    )
    positive_parameters = {
        "material density": density_kg_m3,
        "particle contact offset": material.physx_particle_contact_offset_m,
    }
    for name, value in positive_parameters.items():
        if not math.isfinite(value) or value <= 0.0:
            raise ValueError(f"{name} must be a positive finite number")
    nonnegative_parameters = {
        "viscosity": material.physx_viscosity,
        "cohesion": material.physx_cohesion,
        "adhesion": material.physx_adhesion,
        "surface tension": material.physx_surface_tension,
        "friction": material.physx_friction,
        "damping": material.physx_damping,
    }
    for name, value in nonnegative_parameters.items():
        if not math.isfinite(value) or value < 0.0:
            raise ValueError(f"{name} must be a non-negative finite number")
    effective_bed_center = bed_center_xyz_m or (
        traj.config.bed_center_xy_m[0],
        traj.config.bed_center_xy_m[1],
        traj.config.bed_z_m,
    )
    ...
    script_path.write_text(
        ISAAC_SCRIPT.format(
            ...
            tcp_anchor_candidates=repr(_tcp_anchor_candidates(traj)),
